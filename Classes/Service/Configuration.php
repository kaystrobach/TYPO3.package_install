...
class Tx_PackageInstall_Service_Configuration {
	 ...
	 * @var string
	 */
	private $packageConfigurationPath = 'Configuration/Defaults/localconf.php';
	...
	public function modifyLocalConfFile() {
		...
		$this->InstallerObject->action = str_replace('step='.$this->InstallerObject->step, 'step='.$this->stepAfterConfigurationUpdate . '&packageToInstall=' . t3lib_div::_GP('packageToInstall'), $this->InstallerObject->action);
	...
	public function applyDefaultConfiguration() {
		$this->applyConfigurationFromFile(t3lib_extMgm::extPath('package_install', $this->packageConfigurationPath));
	}
	 ...
	 * @return void
	 */
	public function applySubpackageSpecificConfiguration(Tx_PackageInstall_Model_Package $subpackage) {
		$this->applyConfigurationFromString($subpackage->getZipContent('Install/Configuration/localconf.php'));
	}
	/**
	...
	private function applyConfigurationFromFile($file) {
		$this->applyConfigurationFromString(t3lib_div::getUrl($file));
	}
	/**
	 * Apply the configuration from the given php string
	 *
	 * @param string php code
	 * @return void
	 */

	private function applyConfigurationFromString($packageConfiguration) {
		$localConfigurationLines = $this->InstallerObject->writeToLocalconf_control();

		$packageConfiguration = explode(chr(10),str_replace(chr(13),'',trim($packageConfiguration)));
		...
		while (true) {
			...
			$lineCounter++;
			if($lineCounter > 1000) {
				throw new Exception('Can´t find delimiter in the supplied config file');
			}
		}
